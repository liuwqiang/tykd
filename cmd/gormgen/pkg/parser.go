		...
		for _, spec := range decl.Specs {
			...
			for _, v := range structType.Fields.List {
				...
				if t, _ok := v.Type.(*ast.Ident); _ok {
					...
					if v.Tag != nil {
						if !strings.Contains(v.Tag.Value, "gorm") {
							log.Fatalf("cannot process parseTypes,tag is unknown: %s", v.Names[0].String())
							return false
						}
						if strings.Contains(v.Tag.Value, "time") {
							optionField.FieldType = "time.Time"
						} else if strings.Contains(v.Tag.Value, "json") {
							optionField.FieldType = "datatypes.JSON"
						}
					}
				}
				...
				if len(v.Names) > 0 {
					...
					optionField.ColumnName = TheNamingStrategy.ColumnName("", optionField.FieldName)
