...
var TheNamingStrategy = &schema.NamingStrategy{}
...
func (g *Generator) ParserAST(p *Parser, structs []string) (ret *Generator) {
	for _, v := range structs {
		g.buf[TheNamingStrategy.ColumnName("", v)] = new(bytes.Buffer)
	}
...
func (g *Generator) Generate() *Generator {
	...
	for _, v := range g.structConfigs {
		if _, ok := g.buf[TheNamingStrategy.ColumnName("", v.StructName)]; !ok {
			continue
		}
		if err := outputTemplate.Execute(g.buf[TheNamingStrategy.ColumnName("", v.StructName)], v); err != nil {
			panic(err)
		}
	}
