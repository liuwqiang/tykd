...
import (
	...
	"github.com/liuwqiang/tykd/cmd/mysqlmd/mysql"
...
func init() {
	addr := flag.String("addr", "122.115.55.3:32453", "请输入 db 地址，例如：127.0.0.1:3306\n")
	user := flag.String("user", "root", "请输入 db 用户名\n")
	pass := flag.String("pass", "wftest@231", "请输入 db 密码\n")
	name := flag.String("name", "gateway", "请输入 db 名称\n")
...
func main() {
	...
	if err != nil {
		log.Fatal("query tables of database err ", err)
		return
	}
	for _, table := range tables {
		filepath := "/mnt/d/tykd/model/" + table.Name + "_repo"
		...
		if err != nil {
			log.Fatal("markdown file error ", err.Error())
			return
		}
		...
		if err != nil {
			log.Fatal("create and open model file error ", err.Error())
			return
		}
		fmt.Println("  └── file : ", table.Name+"_repo/gen_model.go")

		modelContent := fmt.Sprintf("package %s%s\n", table.Name, "_repo")
		modelContent += fmt.Sprintln("import (")
		modelContent += fmt.Sprintln(`"time"`)
		modelContent += fmt.Sprintln(`"gorm.io/datatypes"`)
		modelContent += fmt.Sprintln(")")
		...
		for _, info := range columnInfo {
			...
			} else if textType(info.DataType) == "datatypes.JSON" {
				modelContent += fmt.Sprintf("%s %s `%s` // %s\n", capitalize(info.ColumnName), textType(info.DataType), "gorm:\"json\"", info.ColumnComment.String)
			} else {
...
func queryTables(db *gorm.DB, dbName string, tableName string) ([]tableInfo, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			log.Fatal("execute query tables action error,had ignored, detail is ", err.Error())
			continue
		}
...
func queryTableColumn(db *gorm.DB, dbName string, tableName string) ([]tableColumn, error) {
	...
	if err != nil {
		log.Fatal("execute query table column action error, detail is ", err.Error())
		return columns, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			log.Fatal("query table column scan error, detail is ", err.Error())
			return columns, err
		}
